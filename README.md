...
For users at <organization>: a dedicated reference + example MCP config bundle access to Semantic Cube, <org-data-mcp> MCP (Semantic Cube + Data Portal + Journey Data + <event-system>), <org-catalog> MCP (NL→SQL + tag namespace), and OpenMetadata curation. See `references/org-extensions.md` and `mcp/example-org-mcp.json`. Non-<organization> users can ignore — the plugin works without them.
