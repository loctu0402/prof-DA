...
Invoke the `da` skill in **frame mode**. Read these references before acting:
...
- Gate 4: Write `PLANNING.md` → route to next mode (`/prof-DA:query` / `/prof-DA:process` / `/prof-DA:model` / `/prof-DA:insight` / `/prof-DA:automate` / `/prof-DA:report`)
