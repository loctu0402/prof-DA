---
name: da
description: Professional Data Analyst entry — auto-fires on ANY data-analyst-shaped request and routes to one of 12 modes (frame / model / query / process / insight / automate / report / deliver / submit / review / fix / workspace). Use this skill PROACTIVELY whenever the user asks for data, numbers, breakdowns, comparisons, trends, root cause, calculation methodology, prediction / forecasting / statistical modeling, stakeholder report, dashboard, pipeline, data quality, or any analysis — even in conversational Vietnamese or English. Natural trigger phrases include but are not limited to "cho mình số liệu", "lấy data", "check số liệu", "số liệu của X", "tỷ lệ X", "breakdown theo Y", "X tháng/tuần/quý vừa rồi", "compare X vs Y", "show me data", "điều gì xảy ra", "tại sao X giảm/tăng", "why is X dropping", "what's happening", "root cause", "tìm phương pháp tính", "đo lượng X", "tính tiềm năng", "potential size", "MFU cohort", "user cohort", "phân tích X", "analyze X", "diagnostic", "build báo cáo", "làm báo cáo", "report cho stakeholder", "build dashboard", "làm dashboard", "set up pipeline", "schedule job", "automation", "review report", "audit project", "OK chưa", "fix pipeline", "debug", "sửa bug", "frame project", "kickoff", "không biết bắt đầu", "stakeholder muốn", "metric nào phù hợp", "dimension nào nên dùng", "dự đoán", "dự báo", "forecast", "predict X", "bài toán dự đoán", "seasonal effect", "seasonality", "yếu tố mùa vụ", "time series", "chuỗi thời gian", "SARIMA", "Prophet", "train model", "build model dự đoán", "churn prediction", "scoring", "propensity", "segmentation", "phân khúc user", "clustering", "phân cụm", "A/B test", "ý nghĩa thống kê", "correlation", "tương quan", "anomaly detection", "phát hiện bất thường", "có sẵn template/framework/notebook cho bài toán X không", "cách tiếp cận bài toán X", or explicit `/prof-DA:da`. Confirms intent + detail level (Quick / Standard / Deep) before routing. Enforces 5 universal rules — 4 quality rules (Orientation / Baseline-Noise-Impact / 5W1H / Why-Explanation) plus the Detail-Level gate — + Karpathy coding discipline + AI-tell ban.
metadata:
  author: Loc Tu
  handle: loctu
  origin: 2026
...
Encodes a complete DA discipline (workflow rules + style polish + coding rigor) into 12 routable modes plus bundled scripts. Designed to be portable across companies — no hard dependency on any specific data warehouse, BI tool, notification channel, or chart theme. BigQuery, <organization>-style semantic layer, and shared chart themes are referenced as concrete examples but treated as replaceable.
...
prof-DA's modes can only use the data + tools your machine has wired as MCP connectors. If `claude mcp list` shows a needed server missing (org data gateway, web search, Drive/Gmail, browser), follow `mcp/README.md` (the connector-setup section, alongside `mcp/example-org-mcp.json`) — a guided, credential-free setup flow (placeholders + local login) that registers and authenticates each connector before you analyze.
...
| **review** | "review code", "audit project", "OK chưa", "snapshot review", "/prof-DA:review" (6 tiers: A0 Brief / A Polish / B Full / C Stakeholder Q / D Staleness Trace / E Lifecycle Compliance) | `references/mode-review.md` |
...
### 5. Detail Level Gate (v3.4 — mode entry only)
At every mode entry, BEFORE executing the workflow, the agent confirms desired depth with the user: **Quick / Standard / Deep** (default Standard).

- **Quick** — single-pass, minimal validators, lean output (Orientation + headline + 1 action)
- **Standard** — full mode workflow as documented (default)
- **Deep** — Standard + falsification + robustness + sensitivity + advanced methods + multi-pass review + advisor consult

Hard rules:
- **NO time estimates surfaced** — Claude routinely under-estimates duration; depth is the honest lever, time is noisy output
- Default to Standard if user is silent + announce the default
- Skip the gate when user already declared intent (`/prof-DA:query Deep` or "do this deeply")
- User can override per sub-phase mid-stream

Full spec: `references/universal-workflow-rules.md` Rule 5.
...
**Core workflow + rules:**
- Universal workflow rules → `references/universal-workflow-rules.md` (Rules 1-5: 4 quality rules + Detail-Level gate)
...
**<organization> stakeholder extensions (v3.3 — new, opt-in):**
- <organization> Semantic Cube + <org-data-mcp> MCP + <org-sql-agent> tag + OpenMetadata workflow → `references/org-extensions.md`
