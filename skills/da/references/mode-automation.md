...
Invoke when user asks: "automation", "pipeline tự động", "schedule job", "set up cron", "/prof-DA:automate".

This mode is engine-agnostic. Concrete cron / scheduling tools (Airflow, Dagster, Prefect, plain crontab, Claude `/loop`, Claude `/schedule`, <organization> internal cron, GitHub Actions) are treated as **replaceable schedulers** behind the same workflow contract.
