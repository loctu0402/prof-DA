...
1. **Self-check before review.** After drafting, before invoking `/prof-DA:review`. Catches obvious orphan sections.
2. **Review-gate.** First pass in `/prof-DA:review` Sub-mode A or Sub-mode B. The review pass starts by listing the 5 criteria and ruling each pass/fail with one line of evidence.
