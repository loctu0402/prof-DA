...
| 5 | PREDICT (if any) | process | the forecast/prediction model if the report needs one (time-series / an expected-value blend / scoring); backtest + PI. For a **net/difference driver** (e.g. netcash = cashin − cashout), model the COMPONENTS and subtract — never fit the noisy difference directly — and verdict it on z-score, never %gap (see hard rule below) | a model + metrics.json |
...
## Hard rules
- **Volatile / net-difference metric verdict — z-score, NOT %gap (self-aware anomaly rule).** A metric that is a difference of two large flows (netcash = cashin − cashout; any `net_*` / `nc_*`) or whose Expected drifts near zero / flips sign has NO stable scale: `%gap = residual/|Expected|` mechanically explodes (a near-zero Expected + an ordinary move → "%gap −1800%") and over-flags ~half of all days, training the reader to ignore the alert. For these, (1) judge the deviation on **z-score** (or a robust band — MAD / a backtested out-of-sample σ), never %gap; (2) render the deviation in **absolute units + z**, not %; (3) make the **underlying components the primary signal** — when netcash moves, it's because cashin or cashout moved, and the flow says *why* (e.g. cashout down = less outflow = good), which the net hides; (4) **tell the user** this metric is structurally noisy / often negative so a swing is usually normal, not an alarm. Negative/large daily swings are the metric's NORMAL state. (Worked case: a daily net-cash report where %gap flagged ~46% of days as "abnormal" while a z-score verdict flagged ~2.5%, so the %gap alert was almost all noise. Capture the full pattern and your engine/template fix in your workspace knowledge base.)
