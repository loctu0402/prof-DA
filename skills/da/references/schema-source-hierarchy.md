...
|------|--------|--------------|------|-------------|
| **T0** | Owner-curated LLM-grade tag (e.g. <organization> `<org-catalog>.*` tag in OpenMetadata) | Plain-language descriptions + sample queries + business meaning + lineage hint, deliberately written for AI consumption | Free (one tag fetch) | ALWAYS check first if your catalog supports it |
...
2. Check tag list for tags matching `<org-catalog>.*` namespace
...
### <organization> specifics — two MCPs cover T2

| MCP | What it scopes | Tools relevant to schema discovery |
|-----|----------------|------------------------------------|
| `<org-sql-agent>` MCP | Tables in user's BQ access scope across <organization> domains, with domain knowledge layered on | `get_domain_schema` (returns schema + descriptions filtered to user access), `glob_search` (find tables matching pattern in user scope) |
| `<org-data-mcp>` MCP | Semantic Cube + Data Portal + Journey + <event-system>, scoped to user's team membership | `semantic-get-team-data` → `semantic-fetch-meta-by-id` (cube measures + dimensions + descriptions); `data-portal-search-document-content` (find existing docs about a table); `data-portal-list-documents` (browse domain folders); `event-get-allowed-filter-columns` (event schema if applicable) |
...
### Discovery pattern (<organization>)
```
1. <org-catalog> MCP → get_domain_schema(table_or_domain) → check description + access flag
   - If returned schema rich enough, STOP
2. <org-data-mcp> MCP → semantic-get-team-data → semantic-fetch-meta-by-id(datasource)
   - Find cube wrapping the table → read measures + dimensions
3. <org-data-mcp> MCP → data-portal-search-document-content(table_name)
   - Find any docs analysts wrote about this table → richer than catalog alone
```

For non-<organization> orgs: substitute your access-aware metadata MCP (e.g. dbt-mcp, semantic-mcp, dataworks-mcp). If your org has NO access-aware MCP layer, skip T2 and go T1 → T3.
...
Question: What columns does <table> have, and what do they mean, and do I have access?
   │
   ▼
T0 — Does the catalog expose an owner-curated LLM tag (e.g. <org-catalog>.*)?
...
T2 — Does the org have access-aware metadata MCPs (<org-catalog> / <org-data-mcp> / equivalent)?
...
| T1 description contradicts T4 sample (e.g. desc says "cashout" but data is positive deposits) | T4 WINS → curate T1 fix (see org-extensions.md → "OpenMetadata curation playbook") |
...
- `references/org-extensions.md` — <organization>-specific T0/T1/T2 implementations (<org-sql-agent> tag, OpenMetadata, <org-catalog> MCP, <org-data-mcp> MCP semantic cube + data portal)
