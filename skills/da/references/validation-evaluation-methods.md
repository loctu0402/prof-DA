...
When OLS projection is used to derive an expected value or anomaly threshold (<product-b>-style daily forecast, baseline-from-history), window choice dominates the result.
...
- **The baseline MUST capture the same seasonal regime as the period judged.** A flat trailing mean (e.g. "last 7 days") omits weekday / payday / month-end seasonality. If the day under judgment sits in a seasonal regime (payday Friday, weekend, month-end) the flat baseline systematically under- or over-predicts → it manufactures a large "deviation" that is pure seasonality, not anomaly — and an inflated σ (also flat) escalates it to a false alarm. Before trusting an anomaly verdict, ask: *does the expected/baseline see the same calendar regime as the actual?* If not, the magnitude is uninterpretable. Cross-check the observed deviation against the metric's own **same-regime** historical distribution (e.g. prior paydays only) before alarming.
- **Watch for split modeling paths.** When aggregate metrics go through the full seasonal model but sub-drivers fall back to a cruder baseline, the drivers will scream false anomalies on seasonal days while the aggregate stays calm — a tell that the driver path skipped the seasonal terms.

Why these rules — anomaly detection that uses last-N-months blindly will flag a normal-but-post-regime-shift point as anomalous because the slope was learned from a different regime. Past incident (window): <product-b> expected model + structural break = false anomalies; fix was exclude-break + last-3M window. Past incident (seasonal-regime, 2026-06-06): in a daily financial-metrics snapshot, the per-driver sub-metric rows used a flat 7d-mean baseline (no weekday/payday term) while the aggregates used the full seasonal model; on a payday Friday the drivers showed +66-75% "vs expected" and fired false anomaly flags on the outflow sub-channels — the surge was routine payday behavior (the seasonal-modeled aggregate residual was only +2-4%). Fix: promote the drivers into the same seasonal model. Verified by reproducing the gap to 0.3pp, collapsing the residual to ±10% after removing the payday term, and confirming the per-metric same-regime historical band.
