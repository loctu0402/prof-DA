...
Mode-specific references treat company-specific tooling (<organization> <org-sql-agent>, <organization> daily_user_mart_*, <organization> CASA benchmarks) as **examples**, not requirements. The same workflow runs against any SQL engine, any BI stack, any notification channel.
...
Reason: users can't discover full plugin capability by reading overview ("you don't know what you don't know"). Field test 2026-05-15 confirmed: user shipped static <product> report → missed event-tracking + auto-refresh + alert wiring options. Proactive suggestion at exit recovers ~80% of missed capability.
