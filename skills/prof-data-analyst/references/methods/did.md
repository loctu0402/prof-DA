...
## Worked example — <product> cashout intervention

Setup: in March 2026, <organization> launched a notification campaign targeting Tier 3 <product> users (treatment group: ~50k users). The question: did the campaign reduce cashout by Tier 3 users?
...
```python
df = pd.read_csv("output/projects/<product>-march-campaign/panel.csv")
