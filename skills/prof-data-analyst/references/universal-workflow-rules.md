...
- <organization>-specific acronyms (<product>, <product-b>, MP, VA, AI, IS_MP, mart_*, CASA SLTU, CO/CI)
...
- Business threshold (e.g., 1% AUM gap = ~0.5T VND on <product> = "medium")
...
- **Where**: <product> product, all regions, all balance ranges
- **How**: Start with `daily_user_mart` filtered to Tier 3 cohort, run drain-anatomy SQL pattern from `references/framework.md`
...
RIGHT: "Oil price +31% in March (FACT) → transport + food prices ↑ (MECHANISM) → households reallocate cash toward expenses (BEHAVIORAL CHANGE) → withdrawal from <organization> <product> for daily payments (PRODUCT IMPACT) → Payment Cashout +13.2% same period, evidence in daily_user_mart section 4.1 (EVIDENCE)"
