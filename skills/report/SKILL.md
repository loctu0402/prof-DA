...
- HTML SPA / dashboard / diagram / plan: see `output-effective-html.md` — every HTML artifact follows the EFFECTIVE-HTML standard (themeable via `data-theme` + a light/dark `data-mode` toggle; geometry-in-markup / colour-in-tokens / state-in-classes; SVG diagrams styled by CSS classes, zero hard-coded hex). Fork the boilerplate, never freestyle. A hard-coded-hex single-theme page is a defect
...
- HTML artifact (SPA / dashboard / diagram / plan) design contract: `${CLAUDE_PLUGIN_ROOT}/skills/da/references/output-effective-html.md`
