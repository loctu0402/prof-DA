---
name: review
description: 5 sub-modes for review work. Sub-mode A0 Brief (5-min snapshot Ship/Fix/Rebuild verdict). Sub-mode A Delivery Refine (lightweight polish). Sub-mode B Full Project Refine (heavyweight audit: workflow + method maturity + advanced + fact-check + rework plan + user approval gate). Sub-mode C Stakeholder Questioning (formulate questions before analysis). Sub-mode D Staleness Trace (after a change to one asset, trace + sync every dependent asset: doc, plan, AC/DoD, output). Use this skill whenever the user wants to critique, audit, refine, sanity-check, or re-sync existing work after a change. Auto-fires on natural Vietnamese + English. Natural triggers include "review report", "review bài", "review code", "refine deliverable", "audit project", "audit lại", "kiểm tra bài", "kiểm tra report", "OK chưa", "đã ổn chưa", "có thiếu gì không", "bài này được chưa", "góp ý", "feedback bài này", "stakeholder questions", "câu hỏi cho stakeholder", "hỏi gì stakeholder", "snapshot review", "method maturity", "advanced method gì", "có nên dùng DiD", "method này đủ rigor chưa", or explicit /prof-DA:review.
---
